...
(function() {
    ...
    requestAnimationFrame(() => {
      ...
      if (moveDistance < -20) { // Swiping left (next)
        ...
        if (currentIndex < menuSections.length - 1) {
          ...
          // Check if we should auto-navigate based on edge detection
          detectEdgeForAutoNavigation('next', currentIndex);
        }
        ...
        if (currentIndex > 0) {
          ...
          // Check if we should auto-navigate based on edge detection
          detectEdgeForAutoNavigation('prev', currentIndex);
        }
      } else {
  ...
  // Function to detect if menu navigation should auto-trigger based on edge position
  function detectEdgeForAutoNavigation(direction, currentIndex) {
    // Get the horizontal navigation
    ...
    const navItems = Array.from(navContainer.querySelectorAll('.horizontal-nav-item'));
    ...
    // Determine which item to check based on direction
    let targetItem;
    if (direction === 'next') {
      // When swiping left (next), check if the next menu item is at the right edge
      const nextIndex = currentIndex + 1;
      if (nextIndex < menuSections.length) {
        // Find the nav item that corresponds to the next section
        targetItem = navItems.find(item => item.getAttribute('href') === '#' + menuSections[nextIndex]);
      }
    } else {
      // When swiping right (prev), check if the previous menu item is at the left edge
      const prevIndex = currentIndex - 1;
      if (prevIndex >= 0) {
        // Find the nav item that corresponds to the previous section
        targetItem = navItems.find(item => item.getAttribute('href') === '#' + menuSections[prevIndex]);
      }
    }
    ...
    // Check if the item is at the edge and should trigger navigation
    if (direction === 'next' && itemRect.left <= rightEdge && itemRect.left >= rightEdge - itemRect.width) {
      // Item is at the right edge, navigate to next section
      animateDissolveTransition('next', menuSections[currentIndex + 1]);
    } else if (direction === 'prev' && itemRect.right >= leftEdge && itemRect.right <= leftEdge + itemRect.width) {
      // Item is at the left edge, navigate to previous section
      animateDissolveTransition('prev', menuSections[currentIndex - 1]);
    }
  }
